  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      if (currentState === 'Sign Up') {
        const responce = await axios.post(backend_Url + '/api/user/register', { name, email, password })
        ...
        const responce = await axios.post(backend_Url + '/api/user/login', { email, password })
        if (responce.data.success) {
          setToken(responce.data.token)
          localStorage.setItem('token', responce.data.token);
        } else {
